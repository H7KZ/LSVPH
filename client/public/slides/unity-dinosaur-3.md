...
Notes:
Is Kinematic = Rigidbody2D ignoruje fyzikální síly, ale stále registruje kolize. Pohybujeme ho přímo přes transform nebo
velocity.
...
Notes:
Ověřit: dino narazí do kaktusu → (zatím NullReferenceException, protože GameManager neexistuje). To je v pořádku —
opravíme příští lekci.
